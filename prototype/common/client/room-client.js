...
(function(){
  ...
  // 名字搜索排名(选将/选角共用):中文按名字包含;纯字母按拼音(item.py="guan yu",空格分音节)。
  // 拼音匹配对齐音节边界做前缀:gua→关羽/关平、guanyu→关羽(不会误中 huang 黄);首字母≥2位也认:gy→关羽、zgl→诸葛亮。
  // ü 存为 v,查询 lv/lu 都命中吕布。返回名次(越小越靠前),-1=不匹配。索引懒构建缓存在 item._pyIdx。
  function searchRank(item,qraw){
    const q=(qraw||"").trim();
    if(!q) return 0;
    if(/[^a-zA-Z\s]/.test(q)) return item.name.includes(q) ? (item.name.startsWith(q)?0:1) : -1; // 含中文等:名字包含
    const qn=q.toLowerCase().replace(/\s+/g,"");
    if(!item.py) return -1;
    if(!item._pyIdx){ // 各音节边界起的后缀(v/u 两种写法)+ 首字母串
      const syl=item.py.split(" "), sylU=syl.map(s=>s.replace(/v/g,"u"));
      const sufs=[];
      for(let i=0;i<syl.length;i++){ sufs.push([i,syl.slice(i).join("")]); if(sylU[i]!==syl[i]||sylU.slice(i).join("")!==syl.slice(i).join("")) sufs.push([i,sylU.slice(i).join("")]); }
      Object.defineProperty(item,"_pyIdx",{value:{sufs, ini:syl.map(s=>s[0]).join("")},enumerable:false});
    }
    let best=-1;
    for(const [i,s] of item._pyIdx.sufs) if(s.startsWith(qn)){ const r=(i===0?10:20)+i; if(best<0||r<best) best=r; }
    if(best>=0) return best;
    if(qn.length>=2){ const at=item._pyIdx.ini.indexOf(qn); if(at>=0) return 40+at; }
    return -1;
  }
  // 过滤+排序:名次 → 名字短者 → 原序
  function searchList(items,q){
    if(!(q||"").trim()) return items.slice();
    return items.map((h,i)=>[h,searchRank(h,q),i]).filter(x=>x[1]>=0).sort((a,b)=>a[1]-b[1]||a[0].name.length-b[0].name.length||a[2]-b[2]).map(x=>x[0]);
  }

  window.RoomClient={create,esc,toast,defaultBase,randomId,randomCode,connectFormHtml,bindConnectForm,lsGet,lsSet,$,searchRank,searchList};
})();
