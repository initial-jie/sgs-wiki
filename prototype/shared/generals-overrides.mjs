...
export const SKILL_OVERRIDES = {
  ...
  426: { note: "曹纯 缮甲 线下版(OCR 清理已确认:去'主'、'版啤'→'牌')", cardWarn: "缮甲 已校正为线下版(与官方线上描述不同)——以你手上的实体卡为准", skills: {
  ...
  351: { note: "鲍三娘 许身 官方wiki过时", cardWarn: "许身 已校正为线下版(无线上「回复体力至1点」结尾)——以你手上的实体卡为准", skills: {
    "许身": "限定技，当你进入濒死状态时，你可令一名其他角色选择一项：1.其失去所有技能获得“征南”；2.其获得“镇南”。",
  } },
  // 神张角 异兆/肆军/天劫 —— 线下版(比线上更对线下友好)
  227: { note: "神张角 三技 线下版(与线上描述不同)", cardWarn: "异兆/肆军/天劫 为线下版(与官方线上描述不同)——以你手上的实体卡为准", skills: {
  ...
  726: { note: "界郭皇后 矫诏 补全三级(基础+殚心修改1/2)", cardWarn: "矫诏 已补全「殚心」两级修改(OL 原文过简)——机制以你手上的实体卡为准", skills: {
...
export function applyOverrides(list, warn = (m) => console.warn(m)) {
  ...
  for (const h of list) {
    ...
    if (!ov) continue;
    if (ov.cardWarn) h.cardWarn = ov.cardWarn; // ① 技能修正提示:此将技能经校正,与实体卡版本可能不同(查将时房间弹⚠)
